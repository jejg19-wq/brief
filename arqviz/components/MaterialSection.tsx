...
export default function MaterialSection({project,demo,onUpdate}:{project:Project;demo:boolean;onUpdate:(fn:(p:Project)=>Project)=>void}) {
 ...
 async function upload(file:File) { setError('');setBusy(true);try { if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>4*1024*1024)throw new Error('Sube PNG, JPG o WebP de hasta 4 MB.'); let url:string;
 ...
 <button className="btn-ghost" disabled={busy||(project.materialRefs?.length||0)>=6} onClick={()=>input.current?.click()}>{busy?'Subiendo…':'+ Añadir muestra real de material'}</button><input ref={input} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={e=>{if(e.target.files?.[0])upload(e.target.files[0]);e.target.value='';}}/>
